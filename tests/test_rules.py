...
def test_first_twelve_rules_are_implemented_and_remaining_rules_are_pending(
    client,
):
    page = client.get("/rules").get_data(as_text=True)
    ...
    assert tuple(rule.implementation_status for rule in RULES[:12]) == (
        IMPLEMENTED_STATUS,
        IMPLEMENTED_STATUS,
        IMPLEMENTED_STATUS,
        IMPLEMENTED_STATUS,
    ...
    assert page.count(IMPLEMENTED_STATUS) == 12
    assert page.count(IMPLEMENTATION_PENDING_STATUS) == 2
...
def test_rules_documentation_stays_synchronized_with_registry():
    ...
    assert "CC-RULE-001 through CC-RULE-012 are implemented" in (
        normalized_documentation
    )
    assert "CC-RULE-013 through CC-RULE-014 remain implementation pending" in (
        normalized_documentation
    )
